    ...
    (if (not node-ok?)
        ...
        (let [valid-node (get-valid-node node)]
          (if (not valid-node)
              ...
              (let [;; Save current cursor position before moving
                ...
                (when move-ok?
                  (table.insert position-history current-pos))
                (if (not move-ok?)
    ...
    (let [kset vim.keymap.set
      ...
      (when backward-mapping
        (kset :i backward-mapping
              (fn []
                (if (not (smart-tab-backward))
                    (feedkeys (termcodes backward-mapping true true true) :n
                              true)))
